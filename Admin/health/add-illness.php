...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($validation_result->num_rows > 0) {
        // Step 2: If validation passes, insert illness record into pig_health_illnesses
        $sql = "INSERT INTO pig_health_illnesses (tag_number, disease_type, diagnosis_date, notes) 
                VALUES (?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param('ssss', $tag_number, $disease_type, $diagnosis_date, $notes);
        ...
        if ($stmt->execute()) {
            ...
            echo json_encode([
                "success" => false,
                "message" => "Erreur lors de l'ajout de la maladie."
            ]);
        }
